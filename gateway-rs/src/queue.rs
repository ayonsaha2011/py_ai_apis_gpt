...
impl Admission {
    pub fn new(max_heavy: usize, text_max_waiting: usize, video_max_waiting: usize) -> Self {
        Self {
            heavy: Arc::new(Semaphore::new(max_heavy.max(1))),
    ...
    pub async fn acquire_heavy(&self) -> Result<OwnedSemaphorePermit> {
        Ok(self
            .heavy
            .clone()
            .acquire_owned()
            .await
            .map_err(|_| AppError::Unavailable("heavy workload gate closed".into()))?)
    }
    ...
    pub fn stats(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "heavy_available": self.heavy.available_permits(),
        })
    }
